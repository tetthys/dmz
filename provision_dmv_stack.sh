#!/usr/bin/env bash
# ===================================================================================
# DMZ Stack Provisioner (v3) — Final
# - Provisions two KVM VMs (web-vm on br-external, internal-vm on br-internal)
# - Cloud-init: static IP, minimal packages, Docker/Compose, qemu-guest-agent
# - Containers hardened (non-root/read-only/seccomp placeholder)
# - Idempotent and verbose logging
# ===================================================================================
...
: > "$LOG_FILE" || true
log "===== DMZ Provisioner v3 starting ====="
...
# Compose definitions (placeholders you can replace later)
WEB_COMPOSE=$(cat <<'YML'
version: '3.8'
services:
  web:
...
make_user_data() {
  ...
  local outf="${SEED_DIR}/${vm}-user-data.yaml"
  {
    ...
    if [[ "${ENABLE_PW_AUTH}" = "1" ]]; then
      ...
      echo "chpasswd:"
      echo "  list: |"
      echo "    ${LOGIN_USERNAME}:${LOGIN_PASSWORD}"
      echo "  expire: False"
    fi
    ...
    # Netplan: NIC 이름 3종 동시 정의(ens3/enp1s0/eth0) → 실제 존재하는 것만 적용됨
    cat <<EOF
write_files:
...
EOS
    ...
    echo "  - [ bash, -lc, \"netplan apply\" ]"
    echo "  - [ bash, -lc, \"usermod -aG docker ubuntu || true\" ]"
    ...
    if [[ "${DISABLE_PW_ON_BOOT}" = "1" ]]; then
      ...
      echo "  - [ bash, -lc, \"systemctl restart ssh || systemctl restart sshd || true\" ]"
    fi
  } > "$outf"
  dbg "Wrote $outf"
}
